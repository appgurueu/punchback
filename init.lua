...
DEFAULT_PUNCHBACK=1 --Default punchback strenght in blocks - used when tool hasnt got punchback capability
        ...
        for _,y in pairs({1*d,0,2*d}) do
            for _,x in pairs({-0.5*d,0,0.5*d}) do
                for _,z in pairs({-0.5*d,0,0.5*d}) do
                    ...
                    if not (minetest.get_node(floor).name=="air") then --Stop moving when colliding with solid node
                        goto overjump
                    end
                end
            end
